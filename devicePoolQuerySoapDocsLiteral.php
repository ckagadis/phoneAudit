<?php
//Script for use with SOAP docs/literal format
...
$sql1 = "select * from devicepool where devicepool.name = 'addNameOfDevicePool'";
...
{
  $count++;
  $soapClient = new SoapClient("https://hostname:8443/realtimeservice2/services/RISService70?wsdl", array('stream_context' => $context, 'trace'=>true, 'login' => $username,'password'=> $passwd));
  $deviceIP = $soapClient->selectCmDevice(array('StateInfo' => '', 'CmSelectionCriteria' => (array('SelectBy'=>'Name','Status'=>'Registered','SelectItems'=>array('item'=>array('Item'=>$first->name))))));
  foreach($deviceIP as $first)
  {
    if( is_array($first->SelectCmDeviceResult->CmNodes->item) )
    {
      foreach($first->SelectCmDeviceResult->CmNodes->item as $second)
      {
        if($second->ReturnCode == 'Ok')
        {
          foreach($second->CmDevices as $dev)
          {
            ...
            echo "<b>Phone IP Address</b><br>" . $dev->IPAddress->item->IP . "<br>";

            $ip = $dev->IPAddress->item->IP; //Prepare $ip variable for device pool/subnet check
            $ip = ip2long($ip);
            ...
            if(is_array($deviceExtensionsQuery->return->row)) // Checks to see if device has more than one extension.  In either case, print out the calling search space assigned to the extension, if any
            {
              foreach($deviceExtensionsQuery->return->row as $deviceExtensions)
              {
                ...
                if(isset($extensionsCallingSearchSpaceQuery->return->row->name))
                {
                  $extensionCallingSearchSpaceName = $extensionsCallingSearchSpaceQuery->return->row->name;
                }
                if(empty($extensionsCallingSearchSpaceQuery->return->row->name))
                {
              ...
              echo "<hr>";
              if(isset($alertDevicePool) || isset($alertDeviceCSS) || isset($alertExtensionsCSSImplodedDesc))
              {
            ...
            else // Device has only one extension
            {
              ...
              {
                echo "&nbsp;&nbsp;<b>Extension Calling Search Space Name:</b><br>&nbsp;&nbsp" . $extensionCallingSearchSpaceName . "<br>";
                $extensionCallingSearchSpaceString = substr($extensionCallingSearchSpaceName, 2);
                if (strpos($extensionCallingSearchSpaceName, $devicePoolNameString) !== FALSE)
                {
                  echo "&nbsp;&nbsp;<b>Extension Calling Search Space Status:</b> Correct<br>";
                }
